...
##
##  Copyright (C) 2013-<<current-year()>> Fabrice Niessen. All rights reserved.
##
...
- Replace ~if~ statements with ~stopifnot~ to make input validation more concise.
- Rename the ~myFunc~ function to ~my_func~ to follow R's naming conventions.
- Move the constants to the top of the script as for easier modification.
- Convert the date columns to R date format using ~as.Date()~.

** Code

*** Modify Pander output to remove header lines

This section shows how to handle effectively data output and formatting in R,
specifically using the `pander` package for pretty-printing dataframes.

#+begin_src R
## Pretty print the dataframe using pander.
output <- capture.output(pander(df, caption = caption))
...
Execute the following command to install the ~docopt~ package:

: sudo R -e 'install.packages("docopt", repos = "http://cran.us.r-project.org")'

*** Select specific columns for output

To include only specific columns in the output, use the following code:

#+begin_src R
## Select only the specified columns for pander output.
columns_to_keep <- c("table_name", "field_name", "ml_heading", "allow_null", "data_type",
                     "dflt_val", "num_format", "ref_table", "afm_size", "validate_data")
...
#+begin_src awk
# Remove columns Arrival.time and Departure.time.
df <- subset(df, select = -c(Arrival.time, Departure.time))
#+end_src

*** Handle uncertain column existence

Or, if you are unsure whether the specified columns exist in the dataframe, adapt
the code as follows:
...
columns_to_keep <- c("table_name", "field_name", "ml_heading", "allow_null", "data_type",
                     "dflt_val", "num_format", "ref_table", "afm_size", "validate_data")
existing_columns <- intersect(columns_to_keep, names(df))
df <- df[, existing_columns, drop = FALSE]
#+end_src

*** Remove Row Names from the Dataframe

To prevent row names from being included as an extra column in the output:

#+begin_src R
## Remove row names from the dataframe (to prevent the extra column in the
## output).
rownames(df) <- NULL
...
## Remove rows with all missing values (rows that are fully empty).
df <- df[rowSums(is.na(df)) < ncol(df), ]
df <- df[rowSums(is.na(df) | df == "") < ncol(df), ]

## Remove columns with all missing values (columns with only NA values).
...
#+begin_src R
## Sort the dataframe by 'category' and 'equipment_standard' columns.
df <- df[order(df$category, df$equipment_standard), ]
#+end_src

#+begin_src R
## Rename column 'Room code' to 'Room Code'.
names(df)[names(df) == 'Room code'] <- 'Room Code'
#+end_src
...
#+begin_src R
# Get the indices of unique rows in the data frame.
unique_indices <- !duplicated(df)

# Subset the data frame to keep only the unique rows.
df <- df[unique_indices, ]
#+end_src

*** Refactor column names: replace spaces with dots

#+begin_src awk
## Convert column names with spaces to column names with dots.
colnames(df) <- make.names(colnames(df), unique = TRUE)
## refactored:
# Convert column names with spaces to dot-separated names.
...
- ~%in%~ (or ~is.element()~, more readable) ::
     Test membership.
...
Use another data frame in the loop:

#+begin_src R
## Loop over list.
df <- NULL
for (i in seq(ls)) {
  timestamp   <- as.POSIXct(ls[[i]]$timestamp, format="%Y-%m-%dT%H:%M:%OSZ", tz="UTC")
  temperature <- ls[[i]]$temperature
  df <- rbind(df,
              data.frame(timestamp, temperature))
}
#+end_src
...
#+begin_src R
> setwd("d:/Users/fni/.dotfiles/mypackage/")
> document()
> setwd("..")
> install("mypackage")
> ?mypackage
